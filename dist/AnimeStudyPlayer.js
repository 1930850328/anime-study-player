...
function syncPlayerChrome(host, elapsedSeconds, durationSeconds) {
    ...
    const played = host.querySelector('.xgplayer-progress-played');
    if (played) {
        played.style.width = `${percent * 100}%`;
    }
    const point = host.querySelector('.xgplayer-progress-btn');
    if (point) {
...
export const AnimeStudyPlayer = forwardRef(function AnimeStudyPlayer({ url, poster, durationMs, clipStartMs = 0, clipEndMs, segments, knowledgePoints, showRomaji = true, showSubtitleReading = false, showJapaneseSubtitle = true, showChineseSubtitle = true, autoplay = true, themeColor = '#ffc8af', className, onFinish, onError, onReady, onStateChange, }, ref) {
    ...
    const initialSnapshot = useMemo(() => createSnapshot(0, clipStartMs, clipDurationMs, false, false, true, false, 0.8, segments, knowledgePoints), [clipDurationMs, clipStartMs, knowledgePoints, segments]);
        ...
        const initializePlayer = async () => {
            ...
            const missingPictureGuard = () => {
                if (!renderedFrameRef.current &&
                    ...
                    media.videoWidth === 0) {
                    player.pause();
                    const message = '当前文件已经开始播放音频，但画面没有正常解码。建议先转成 MP4(H.264/AAC) 再播放。';
                    setPlayerError(message);
                    onErrorRef.current?.(message);
                }
            };
